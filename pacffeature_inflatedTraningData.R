...
for (i in 1:2){ # dog.type i <- 2
  for (j in 1:dog.count[i]){ # dog.count j <- 1
    ...
    path1 <- paste0(dog.type[i], "_", j, "_pacf50.csv" )
    #path2 <- paste0("K:/Yet/output/",dog.type[i], "_", j, "_pacf50.csv" )
    write.csv(d, file = path1, row.names = F)
    # d <- read.csv("Dog_1_pacf50.csv")
    #first melt
    ...
    if (i ==2)cast.d = dcast(melt.d,Patient +Seizure+File ~ varname,value.var = 'value')
    ...
    rf.d3 = rf.d[index,]# str(rf.d3) colnames(rf.d3)
    ...
    path3 <- paste0(dog.type[i], "_", j, "_predpacf50.csv" )
...
proc.time() -pm1
## another idea is cloning the data, i.e, augmented the number of interictal and
## preictal in each subject using resampling procedure to make 
## the number of interictal segment are the same across subject
## the number of preictal segments are the s